...
export default function AnalyticsPage() {
  ...
  const maxBar = Math.max(...weeklyData.map((d) => Math.max(d.completed, d.created)), 1);
  ...
  const STAT_CARDS = [
    { label: 'Total Tasks',      value: stats.totalTasks,          icon: '/icon-check.png',     bg: 'bg-cream-100' },
    { label: 'Completed',        value: stats.completedTasks,       icon: '/icon-check.png',     bg: 'bg-moss-50'   },
    { label: 'Completion Rate',  value: `${stats.avgCompletionRate}%`, icon: '/icon-analytics.png', bg: 'bg-cream-200' },
    { label: 'Overdue',          value: stats.overdueTasks,         icon: '/icon-priority.png',  bg: stats.overdueTasks > 0 ? 'bg-red-50' : 'bg-cream-100' },
    { label: 'Best Day',         value: stats.bestDay,              icon: '/float-star-orange.png', bg: 'bg-amber-50' },
    { label: 'Longest Streak',   value: `${stats.longestStreak}d`,  icon: '/ic-fire.png',        bg: 'bg-orange-50' },
  ];
    ...
    <div className="flex flex-col gap-7 animate-fade-in">
      ...
      <div>
        <h1 className="text-2xl font-extrabold text-bark-600 flex items-center gap-2" style={{ fontFamily: "'Baloo 2', cursive" }}>
          Analytics
          <Image src="/icon-analytics.png" alt="analytics" width={28} height={28} className="object-contain" />
        </h1>
      ...
      <div className="card">
        <div className="flex items-center gap-2 mb-6">
          <Image src="/icon-analytics.png" alt="chart" width={20} height={20} className="object-contain" />
          <h2 className="text-lg font-extrabold text-bark-600" style={{ fontFamily: "'Baloo 2', cursive" }}>Last 7 Days</h2>
            ...
            <div key={day.date} className="flex-1 flex flex-col items-center gap-1">
              <div className="w-full flex items-end gap-1 h-40">
                <div className="flex-1 flex flex-col justify-end">
                  <div className="w-full bg-cream-300 rounded-t-lg transition-all duration-700"
                    style={{ height: `${maxBar > 0 ? (day.created / maxBar) * 100 : 0}%`, minHeight: day.created > 0 ? '8px' : '0' }} />
                </div>
                <div className="flex-1 flex flex-col justify-end">
                  <div className="w-full bg-moss-400 rounded-t-lg transition-all duration-700"
      ...
      <div className="card">
        <div className="flex items-center gap-2 mb-5">
          <Image src="/icon-daily.png" alt="habits" width={20} height={20} className="object-contain" />
        ...
        <div className="grid grid-cols-10 gap-1.5">
          {heatmapData.map((day) => (
            <div key={day.date} className="aspect-square rounded-md cursor-default transition-transform hover:scale-110 group relative"
      ...
      {categoryData.length > 0 && (
        <div className="card">
          <div className="flex items-center gap-2 mb-5">
            <Image src="/icon-calendar.png" alt="categories" width={20} height={20} className="object-contain" />
            <h2 className="text-lg font-extrabold text-bark-600" style={{ fontFamily: "'Baloo 2', cursive" }}>Tasks by Category</h2>
          </div>
      ...
      {stats.totalTasks > 0 && (
        <div className="card bg-gradient-to-br from-moss-50 to-cream-100 border-moss-200">
          <h2 className="text-lg font-extrabold text-bark-600 mb-4" style={{ fontFamily: "'Baloo 2', cursive" }}>Overall Summary</h2>
          <div className="flex items-center gap-6">
            ...
            <div className="flex flex-col gap-2">
              ...
              <div className="flex items-center gap-2 mt-1">
                <Image src="/ic-fire.png" alt="streak" width={14} height={14} className="object-contain" />
