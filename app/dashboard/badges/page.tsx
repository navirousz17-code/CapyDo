    ...
    <div className="flex flex-col gap-6 animate-fade-in">
      <div>
        <h1 className="text-2xl font-extrabold text-bark-600 flex items-center gap-2" style={{ fontFamily: "'Baloo 2', cursive" }}>
          Achievements
          <Image src="/icon-badge.png" alt="badges" width={28} height={28} className="object-contain" />
        </h1>
        <p className="text-bark-400 text-sm font-medium mt-0.5">{earnedCount}/{BADGE_DEFINITIONS.length} badges unlocked</p>
      </div>
      ...
      <div className="card">
        ...
        <div className="h-3 bg-cream-200 rounded-full overflow-hidden">
          <div className="h-full rounded-full transition-all duration-700"
            style={{ width: `${(earnedCount / BADGE_DEFINITIONS.length) * 100}%`, background: 'linear-gradient(90deg, #f59e0b, #f97316)' }} />
        </div>
        {earnedCount === BADGE_DEFINITIONS.length && (
          <p className="text-center text-sm font-bold text-amber-600 mt-3">🎊 Complete collection! You're a legend! 🎊</p>
        )}
      </div>
            ...
            return (
              <div key={badge.id}
                ...
                <div className="flex justify-center mb-3 relative">
                  {isEarned ? (
                    <Image src={badge.image} alt={badge.name} width={90} height={90} className="drop-shadow-lg" />
                  ) : (
                    <div className="w-24 h-24 rounded-full bg-cream-200 flex items-center justify-center relative">
                      <Image src={badge.image} alt={badge.name} width={90} height={90} className="opacity-20" />
                ...
                {isEarned && <div className="absolute top-2 right-2 text-lg">✨</div>}
              </div>
            );
          })}
        </div>
