...
import { Plus, Trash2, X, CheckCircle2, Circle, Pencil } from 'lucide-react';
    ...
    <div className="flex flex-col gap-6 animate-fade-in">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-extrabold text-bark-600 flex items-center gap-2" style={{ fontFamily: "'Baloo 2', cursive" }}>
            Daily Activities
            <Image src="/icon-daily.png" alt="daily" width={28} height={28} className="object-contain" />
          </h1>
        ...
        <div className="card bg-gradient-to-r from-moss-50 to-cream-100 border-moss-200">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              <Image src="/icon-check.png" alt="progress" width={20} height={20} className="object-contain" />
        ...
        <div className="card animate-slide-up">
          <h2 className="text-lg font-extrabold text-bark-600 mb-5 flex items-center gap-2" style={{ fontFamily: "'Baloo 2', cursive" }}>
            {editingId ? (
              ...
              <><Image src="/float-sparkle.png" alt="new" width={20} height={20} className="object-contain" /> New Daily Activity</>
            )}
          </h2>
          <form onSubmit={handleSubmit} className="flex flex-col gap-5">
            <div>
              ...
              <input type="text" value={title} onChange={(e) => setTitle(e.target.value)}
                placeholder="e.g. Drink 8 glasses of water" className="input-field" autoFocus maxLength={80} />
            </div>
              ...
              <div className="grid grid-cols-10 gap-1.5">
                {ACTIVITY_ICONS.map((ic) => (
                  <button key={ic} type="button" onClick={() => setIcon(ic)}
                    className={cn('w-9 h-9 text-lg rounded-lg flex items-center justify-center transition-all border',
            ...
            <div className="flex gap-3">
              ...
              <button type="submit" disabled={saving || !title.trim()} className="btn-primary flex-1 flex items-center justify-center gap-2">
                {editingId ? <><Pencil size={14} /> Update</> : 'Add Activity'}
              </button>
            </div>
          </form>
        </div>
      ...
      {loading ? (
        ...
        <div className="card text-center py-16">
          <Image src="/icon-daily.png" alt="no activities" width={56} height={56} className="object-contain mx-auto mb-4" />
        ...
        <div className="flex flex-col gap-3">
          {stats.completedToday === stats.total && stats.total > 0 && (
            <div className="card bg-gradient-to-r from-moss-100 to-cream-100 border-moss-300 text-center py-5 animate-bounce-in">
              <Image src="/float-star.png" alt="done" width={48} height={48} className="object-contain mx-auto mb-2" />
          ...
          {activities.map((activity) => (
            <div key={activity.id}
              className={cn('card card-lift flex items-center gap-4 group transition-all', activity.completed_today && 'opacity-75')}
              style={{ borderLeft: `4px solid ${activity.color}` }}>
              <button onClick={() => toggleActivity(activity.id)} className="flex-shrink-0 transition-transform hover:scale-110 active:scale-95">
              ...
              <div className="flex items-center gap-3 flex-1 min-w-0">
                ...
                <div className="min-w-0">
                  <p className={cn('font-bold text-bark-600 truncate', activity.completed_today && 'line-through text-bark-400')}>
                    {activity.title}
                  </p>
                  <div className="flex items-center gap-2 mt-0.5">
                    ...
                    {(activity.streak ?? 0) > 0 && (
                      <span className="flex items-center gap-1 text-xs font-bold text-amber-500">
                        <Image src="/ic-fire.png" alt="streak" width={12} height={12} className="object-contain" /> {activity.streak} day streak
                      </span>
                    )}
                  </div>
                </div>
      ...
      {activities.length > 0 && (
        <div className="flex items-center gap-3 p-4 bg-cream-100 rounded-xl border border-cream-200">
          <Image src="/icon-sync.png" alt="reset" width={20} height={20} className="object-contain flex-shrink-0" />
