    ...
    <div className="flex flex-col gap-6 animate-fade-in max-w-2xl mx-auto">
      ...
      {tab === 'time' && (
        <div className="flex flex-col gap-5">
          ...
          {activeLog && (
            <div
              className="card flex items-center gap-4 animate-bounce-in"
          ...
          <div className="grid grid-cols-3 gap-3">
            {[
              { label: 'Today', value: formatDuration(totalToday), icon: '📅' },
              { label: 'Sessions', value: logs.filter((l) => l.ended_at).length, icon: '✅' },
              { label: 'Active', value: activeLog ? '1' : '0', icon: '🔴' },
            ].map((s) => (
          ...
          {/* Log history */}
          <div className="card">
            <h2 className="font-extrabold mb-4" style={{ fontFamily: "'Baloo 2', cursive", color: 'var(--text-primary)' }}>
              📋 History
            </h2>
            {loading ? (
                  ...
                  <div key={date}>
                    ...
                    <div className="flex flex-col gap-2">
                      {dateLogs.filter((l) => l.ended_at).map((log) => (
                        <div key={log.id} className="flex items-center gap-3 p-3 rounded-xl group" style={{ backgroundColor: 'var(--bg-secondary)' }}>
                          <Clock size={14} style={{ color: 'var(--text-muted)' }} className="flex-shrink-0" />
                          <div className="flex-1 min-w-0">
                          ...
                          <span className="font-extrabold text-sm flex-shrink-0" style={{ fontFamily: "'Baloo 2', cursive", color: 'var(--success)' }}>
      ...
      {tab === 'mood' && (
        <div className="flex flex-col gap-5">
          {/* Today's mood */}
          <div className="card">
            <h2 className="font-extrabold mb-1" style={{ fontFamily: "'Baloo 2', cursive", color: 'var(--text-primary)' }}>
            ...
            >
              {savingMood ? <Loader2 size={16} className="animate-spin" /> : '🌿'}
              ...
              <>
                ...
                <div className="flex items-end gap-1.5 h-16 mb-4">
                  {moods.slice(0, 14).reverse().map((m) => {
                    ...
                    return (
                      <div key={m.id} className="flex-1 flex flex-col items-center gap-1" title={`${moodConfig.label} — ${m.date}`}>
                        <div
          ...
          >
            💡 Tracking your mood daily helps you spot patterns — maybe you're most productive on Tuesdays, or your mood dips mid-week. Knowledge is power! 🌿
          </div>
        </div>
      )}
    </div>
