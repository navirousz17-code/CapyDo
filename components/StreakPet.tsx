  ...
  return (
    <>
      ...
      <div className="relative w-full rounded-3xl overflow-hidden"
        style={{ background:'var(--bg-card)', border:'1.5px solid var(--border)', boxShadow:`0 8px 48px ${auraColor}, 0 2px 8px rgba(0,0,0,0.05)` }}>
        ...
        <div className="relative flex flex-col sm:flex-row items-center gap-6 p-6 md:p-8">
          <div className="relative flex-shrink-0 flex items-center justify-center" style={{ width:160, height:160 }}>
            ...
            <div className={pulse ? 'card-hero-pulse' : 'card-hero-float'}
              style={{ position:'relative', zIndex:2, filter:stage.stage==='adult'?'drop-shadow(0 0 22px rgba(255,160,0,0.85))':stage.stage==='teen'?'drop-shadow(0 0 18px rgba(140,60,255,0.85))':'drop-shadow(0 8px 20px rgba(0,0,0,0.18))' }}>
          ...
          <div className="flex-1 w-full flex flex-col gap-4">
            <div>
              ...
              {message && <p className="text-sm mt-1 italic" style={{ color:'var(--text-muted)' }}>"{message}"</p>}
            </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-sm font-black" style={{ color:'var(--text-primary)', fontFamily:"'Baloo 2', cursive" }}>{xp} XP</span>
                </div>
              ...
              {PET_STAGES.map((s, i) => (
                <div key={s.stage} className="relative flex-1 flex flex-col items-center gap-1" title={s.name}>
                  <div className="w-full h-1.5 rounded-full transition-all duration-500"
                    style={{ backgroundColor:i<=stageIndex?(stage.stage==='adult'?'#f59e0b':stage.stage==='teen'?'#a855f7':'var(--accent)'):'var(--bg-secondary)', opacity:i===stageIndex?1:i<stageIndex?0.65:0.25 }} />
                  {i === stageIndex && (
                    <div className="absolute -top-1 left-1/2 -translate-x-1/2 w-3 h-3 rounded-full border-2"
                      style={{ backgroundColor:stage.stage==='adult'?'#f59e0b':stage.stage==='teen'?'#a855f7':'var(--accent)', borderColor:'var(--bg-card)', boxShadow:`0 0 8px ${glowColor}` }} />
                  )}
                </div>
              ))}
            </div>
            <div className="flex justify-between -mt-1 px-0.5">
              {PET_STAGES.map((s) => (
                <span key={s.stage} className="flex-1 text-center text-[9px] font-bold truncate"
                  ...
                  <Image src={`/stage_${s.stage}.png`} alt={s.name} width={20} height={20} className="object-contain rounded-full" style={{ opacity:s.stage===stage.stage?1:0.4 }} />
                </span>
              ))}
            </div>
    ...
    <>
      ...
      {showEvolutionBanner && pendingEvolution && (
        <AchievementAnimation
          type="evolution"
          title={pendingEvolution.name}
          subtitle={pendingEvolution.vibe}
          image={pendingEvolution.image}
          onDismiss={dismissEvolutionBanner}
        />
      )}
        ...
        <div className="relative">
          <button
            ...
            style={{
              ...
              filter: stage.stage==='adult'?'drop-shadow(0 0 10px rgba(255,160,0,0.75))':stage.stage==='teen'?'drop-shadow(0 0 10px rgba(140,60,255,0.75))':'drop-shadow(0 4px 12px rgba(0,0,0,0.22))',
            }}>
